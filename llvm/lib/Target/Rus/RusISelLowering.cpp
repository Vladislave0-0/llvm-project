...
#include "Rus.h"
#include "RusMachineFunctionInfo.h"
...
SDValue RusTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Rus);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, Rus::ZAKROM, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(RusISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_Rus);
...
/// Rus formal arguments implementation
SDValue RusTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Rus);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &Rus::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    RusFunctionInfo *UFI = MF.getInfo<RusFunctionInfo>();
...
bool RusTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_Rus))
    return false;
...
RusTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Rus);
  ...
  return DAG.getNode(RusISD::RET, DL, MVT::Other, RetOps);
}
